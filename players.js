...
function getPlayer(id, className = 'archer') {
    if (!players[id]) {
        players[id] = {
            id: id, name: null, class: className, vip: false, vipExpires: null,
            level: 1, xp: 0, gold: 0, nox: 0, glorias: 0,
            energy: 20, maxEnergy: 20, lastEnergy: Date.now(),
            hp: baseStats[className].hp, maxHp: baseStats[className].hp,
            atk: baseStats[className].atk, def: baseStats[className].def, crit: baseStats[className].crit,
            equipment: { weapon: null, armor: null, helmet: null, boots: null, ring: null, necklace: null, bag: null },
            souls: [null, null], inventory: [], maxInventory: 20,
            skin: null, skins: [],
            consumables: { potionHp: 3, potionEnergy: 0, tonicStrength: 0, tonicDefense: 0, tonicPrecision: 0 },
            keys: 3, keyLastDaily: null, lastDaily: null,
...
function savePlayer(id, data) { players[id] = data; }
...
function recalculateStats(player) {
    ...
    for (const slot in player.equipment) {
        ...
        if (item) {
            ...
            totalHp += item.hp || 0;
            if (slot === 'bag' && item.extraSlots) player.maxInventory = 20 + item.extraSlots;
        }
    }
