// Funções puras de combate (sem estado)
...
function playerAttack(state) {
    // Aplica buffs
    let playerAtk = state.player.atk;
    let playerDef = state.player.def;
    let playerCrit = state.player.crit;
    
    if (state.player.buffAtk && state.player.buffAtk.duration > 0) {
        playerAtk = Math.floor(playerAtk * (1 + state.player.buffAtk.value / 100));
        state.player.buffAtk.duration--;
    }
    if (state.player.buffDef && state.player.buffDef.duration > 0) {
        playerDef = Math.floor(playerDef * (1 + state.player.buffDef.value / 100));
        state.player.buffDef.duration--;
    }
    if (state.player.buffCrit && state.player.buffCrit.duration > 0) {
        playerCrit += state.player.buffCrit.value;
        state.player.buffCrit.duration--;
    }
