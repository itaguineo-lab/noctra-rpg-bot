const maps = [
    { name: "Clareira Sombria", level: 1, enemies: [
    ...
    { name: "Cripta em Ruínas", level: 8, enemies: [
    ...
    { name: "Pântano Corrompido", level: 15, enemies: [
    ...
    { name: "Deserto Incandescente", level: 24, enemies: [
    ...
    { name: "Montanhas Gélidas", level: 35, enemies: [
...
function getMap(player) { return maps.find(m => player.level >= m.level) || maps[0]; }
function getMapByName(name) { return maps.find(m => m.name === name); }
function getRandomEnemy(player) {
    const map = getMap(player);
    const available = map.enemies.filter(e => player.level >= e.minLevel);
    return available.length ? available[Math.floor(Math.random() * available.length)] : map.enemies[0];
}
